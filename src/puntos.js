...
export function clasificacionGrupo(equipos, partidosGrupo, resultados, desempates) {
  ...
  // Aplicar desempate manual si lo hay
  if (desempates) {
    // Detectar empates exactos consecutivos
    let i = 0;
    while (i < arr.length) {
      const grupo = [arr[i]]; let j = i + 1;
      while (j < arr.length && arr[j].pts === arr[i].pts && arr[j].dg === arr[i].dg && arr[j].gf === arr[i].gf) {
        grupo.push(arr[j]); j++;
      }
      if (grupo.length > 1) {
        const key = grupo.map(e => e.equipo).sort().join('|');
        const orderStr = desempates[key];
        if (orderStr) {
          const desired = orderStr.split('|');
          const ordered = desired.map(name => grupo.find(e => e.equipo === name)).filter(Boolean);
          if (ordered.length === grupo.length) {
...
export function calcularPuntos(porra, real, constantes) {
  ...
  Object.entries(GRUPOS).forEach(([letra, equipos]) => {
    ...
    const clasifReal = clasificacionGrupo(equipos, partidosG, rRes, rDes && rDes[letra]);
    const clasifPred = clasificacionGrupo(equipos, partidosG, pRes, pDes && pDes[letra]);
  ...
  function resolverCruces(KO_res, des, desT) {
    ...
    Object.entries(GRUPOS).forEach(([letra, equipos]) => {
      ...
      clas[letra] = clasificacionGrupo(equipos, pGr, rRes, des && des[letra]);
    });
    // Mejores terceros
    const terceros = [];
  ...
  function resolverPred() {
    ...
    Object.entries(GRUPOS).forEach(([letra, equipos]) => {
      const pGr = PARTIDOS.filter(p => p.grupo === letra);
      clas[letra] = clasificacionGrupo(equipos, pGr, pRes, pDes && pDes[letra]);
    });
    const terceros = [];
    Object.entries(clas).forEach(([letra, tabla]) => {
      if (tabla[2] && tabla[2].pj === 3) terceros.push({ ...tabla[2], grupo: letra });
